...
public:
    ...
    app * mk_add(unsigned num_args, expr * const * args) const {
        SASSERT(num_args > 0);
        return num_args == 1 && is_app(args[0]) ? to_app(args[0]) : m_manager.mk_app(arith_family_id, OP_ADD, num_args, args);
    }
    ...
    app * mk_sub(unsigned num_args, expr * const * args) const {
        SASSERT(num_args > 0);
        return m_manager.mk_app(arith_family_id, OP_SUB, num_args, args);
    }
    ...
    app * mk_mul(unsigned num_args, expr * const * args) const {
        SASSERT(num_args > 0);
        return num_args == 1 && is_app(args[0]) ? to_app(args[0]) : m_manager.mk_app(arith_family_id, OP_MUL, num_args, args);
    }
