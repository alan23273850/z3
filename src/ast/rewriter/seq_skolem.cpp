...
expr_ref skolem::mk_nq_string(const std::pair<int, int> &id, int part) {
    return expr_ref(seq.mk_skolem(symbol("seq.nq_string"), 3,
        std::initializer_list<expr*>({a.mk_int(id.first), a.mk_int(id.second), a.mk_int(part)}).begin(),
