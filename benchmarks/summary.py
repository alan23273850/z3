...
SAT = int(subprocess.check_output(f"grep _sat statistics/{sys.argv[1]}.csv|wc -l", shell=True).decode("utf8").split()[0])
sat = int(subprocess.check_output(f"grep ,sat statistics/{sys.argv[1]}.csv|wc -l", shell=True).decode("utf8").split()[0])
print ('SAT: ', sat, '/', SAT)
UNSAT = int(subprocess.check_output(f"grep _unsat statistics/{sys.argv[1]}.csv|wc -l", shell=True).decode("utf8").split()[0])
