...
def from_file_to_data_row(file, timeout):
    # print(file)
    ...
    p = subprocess.run(f"time -p timeout {timeout} ./z3 {file}", shell=True, capture_output=True, executable='/bin/bash')
    ...
    try:
        ...
        if 'unknown' in stdoutList:
            if res: raise Exception()
            res = 'unknown'
        if float(t) >= timeout: res = 'timeout'
    ...
    except: res = 'timeout' if float(t) >= timeout and not msg.endswith('(C)ontinue, (A)bort, (S)top, (T)hrow exception, Invoke (G)DB') else 'error'
    ...
    #     "time -p timeout {timeout} ~/cvc4-1.8-x86_64-linux-opt --lang=smt --strings-exp -m", shell=True, capture_output=True, executable='/bin/bash')
    ...
    #             else:
    #                 if line == '(model': read_model = True
    # except: res = 'timeout' if float(t) >= timeout else 'error'
    # result_list += [t, res]
    ########################################
    ...
    # p = subprocess.run(f"cat {file} | cat - <(echo ''; echo '(get-model)') |" \
    #     r"sed 's/(set-logic[^)]*)//g; s/(set-option[^)]*:strings-exp[^)]*)//g; s/str\.to_int/str.to.int/g; s/str\.from_int/int.to.str/g; s/str\.in_re/str.in.re/g; s/str\.to_re/str.to.re/g'" + f" > {file}.output && " \
    #     # f"time -p timeout {timeout} ~/z3/trau-build/z3 smt.string_solver=trau {file}.output", shell=True, capture_output=True, executable='/bin/bash')
    #     f"time -p ~/z3/trau-build/z3 smt.string_solver=trau {file}.output", shell=True, capture_output=True, executable='/bin/bash')
    # import os; os.system(f'rm {file}.output')
    # t = p.stderr.splitlines()[-3].decode('utf-8'); assert t.startswith('real '); t = t.split(' ')[1]
    # try:
    #     for line in p.stdout.splitlines():
    #         line = line.decode('utf-8').strip()
    ...
    #             read_model = True
    # except: pass
    # if res2 not in ('sat', 'unsat', 'unknown') and float(t) >= timeout:
    #     res2 = 'timeout'
    ...
    #         r"sed 's/(set-logic[^)]*)/(set-logic ALL)/g; s/(set-option[^)]*:strings-exp[^)]*)//g' | timeout {timeout} ~/z3-4.8.9 -in", shell=True, capture_output=True, executable='/bin/bash')
    ...
    #         r"sed 's/(set-logic[^)]*)/(set-logic ALL)/g; s/(set-option[^)]*:strings-exp[^)]*)//g; s/str\.to\.int/str.to_int/g; s/int\.to\.str/str.from_int/g; s/str\.in\.re/str.in_re/g; s/str\.to\.re/str.to_re/g; s/\\\\/\\z/g;" \
    #         r"s/\\\([[:digit:]]\)/\\u{\1}/g; s/\\x\([[:xdigit:]][[:xdigit:]]\)/\\u{\1}/g; s/\\a/\\u{7}/g; s/\\b/\\u{8}/g; s/\\f/\\u{c}/g; s/\\n/\\u{a}/g; s/\\r/\\u{d}/g; s/\\t/\\u{9}/g; s/\\v/\\u{b}/g; s/\\z/\\/g' |" \
    #         "timeout {timeout} ~/cvc4-1.8-x86_64-linux-opt --lang=smt --strings-exp", shell=True, capture_output=True, executable='/bin/bash')
    ...
    #         "timeout {timeout} ~/z3/trau-build/z3 smt.string_solver=trau -in", shell=True, capture_output=True, executable='/bin/bash')
    #         # f"time -p timeout {timeout} ~/z3/trau-build/z3 smt.string_solver=trau {file}", shell=True, capture_output=True, executable='/bin/bash')
    #     try:
